...
const AppCompat: Partial<typeof App> = {}
    ...
    const interceptor = new class extends PaperbackInterceptor {
        ...
        override async interceptRequest(request: PBRequest): Promise<PBRequest> {
            ...
            const oldRequest: Request = {
                ...
                cookies: Object.keys(request.cookies ?? {}).map(x => ({ name: x, value: request.cookies![x]!, domain: '' }))
            }
        ...
        override async interceptResponse(request: PBRequest, response: PBResponse, data: ArrayBuffer): Promise<ArrayBuffer> {
    ...
    return {
        ...
        async schedule(request, retry) {
            ...
            return {
                ...
                get rawData() { return new Uint8Array(data) as RawData }
            }
        },
    }
}
