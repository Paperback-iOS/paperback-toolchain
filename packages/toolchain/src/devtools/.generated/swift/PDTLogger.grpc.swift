//
// DO NOT EDIT.
// swift-format-ignore-file
//
// Generated by the protocol buffer compiler.
// Source: PDTLogger.proto
//
import GRPC
import NIO
import NIOConcurrencyHelpers
import SwiftProtobuf


/// Usage: instantiate `PaperbackLoggerClient`, then call methods of this protocol to make API calls.
public protocol PaperbackLoggerClientProtocol: GRPCClient {
  var serviceName: String { get }
  var interceptors: PaperbackLoggerClientInterceptorFactoryProtocol? { get }

  func streamLogs(
    _ request: LogFilter,
    callOptions: CallOptions?,
    handler: @escaping (LogLine) -> Void
  ) -> ServerStreamingCall<LogFilter, LogLine>
}

extension PaperbackLoggerClientProtocol {
  public var serviceName: String {
    return "PaperbackLogger"
  }

  /// Server streaming call to streamLogs
  ///
  /// - Parameters:
  ///   - request: Request to send to streamLogs.
  ///   - callOptions: Call options.
  ///   - handler: A closure called when each response is received from the server.
  /// - Returns: A `ServerStreamingCall` with futures for the metadata and status.
  public func streamLogs(
    _ request: LogFilter,
    callOptions: CallOptions? = nil,
    handler: @escaping (LogLine) -> Void
  ) -> ServerStreamingCall<LogFilter, LogLine> {
    return self.makeServerStreamingCall(
      path: PaperbackLoggerClientMetadata.Methods.streamLogs.path,
      request: request,
      callOptions: callOptions ?? self.defaultCallOptions,
      interceptors: self.interceptors?.makestreamLogsInterceptors() ?? [],
      handler: handler
    )
  }
}

@available(*, deprecated)
extension PaperbackLoggerClient: @unchecked Sendable {}

@available(*, deprecated, renamed: "PaperbackLoggerNIOClient")
public final class PaperbackLoggerClient: PaperbackLoggerClientProtocol {
  private let lock = Lock()
  private var _defaultCallOptions: CallOptions
  private var _interceptors: PaperbackLoggerClientInterceptorFactoryProtocol?
  public let channel: GRPCChannel
  public var defaultCallOptions: CallOptions {
    get { self.lock.withLock { return self._defaultCallOptions } }
    set { self.lock.withLockVoid { self._defaultCallOptions = newValue } }
  }
  public var interceptors: PaperbackLoggerClientInterceptorFactoryProtocol? {
    get { self.lock.withLock { return self._interceptors } }
    set { self.lock.withLockVoid { self._interceptors = newValue } }
  }

  /// Creates a client for the PaperbackLogger service.
  ///
  /// - Parameters:
  ///   - channel: `GRPCChannel` to the service host.
  ///   - defaultCallOptions: Options to use for each service call if the user doesn't provide them.
  ///   - interceptors: A factory providing interceptors for each RPC.
  public init(
    channel: GRPCChannel,
    defaultCallOptions: CallOptions = CallOptions(),
    interceptors: PaperbackLoggerClientInterceptorFactoryProtocol? = nil
  ) {
    self.channel = channel
    self._defaultCallOptions = defaultCallOptions
    self._interceptors = interceptors
  }
}

public struct PaperbackLoggerNIOClient: PaperbackLoggerClientProtocol {
  public var channel: GRPCChannel
  public var defaultCallOptions: CallOptions
  public var interceptors: PaperbackLoggerClientInterceptorFactoryProtocol?

  /// Creates a client for the PaperbackLogger service.
  ///
  /// - Parameters:
  ///   - channel: `GRPCChannel` to the service host.
  ///   - defaultCallOptions: Options to use for each service call if the user doesn't provide them.
  ///   - interceptors: A factory providing interceptors for each RPC.
  public init(
    channel: GRPCChannel,
    defaultCallOptions: CallOptions = CallOptions(),
    interceptors: PaperbackLoggerClientInterceptorFactoryProtocol? = nil
  ) {
    self.channel = channel
    self.defaultCallOptions = defaultCallOptions
    self.interceptors = interceptors
  }
}

@available(macOS 10.15, iOS 13, tvOS 13, watchOS 6, *)
public protocol PaperbackLoggerAsyncClientProtocol: GRPCClient {
  static var serviceDescriptor: GRPCServiceDescriptor { get }
  var interceptors: PaperbackLoggerClientInterceptorFactoryProtocol? { get }

  func makeStreamLogsCall(
    _ request: LogFilter,
    callOptions: CallOptions?
  ) -> GRPCAsyncServerStreamingCall<LogFilter, LogLine>
}

@available(macOS 10.15, iOS 13, tvOS 13, watchOS 6, *)
extension PaperbackLoggerAsyncClientProtocol {
  public static var serviceDescriptor: GRPCServiceDescriptor {
    return PaperbackLoggerClientMetadata.serviceDescriptor
  }

  public var interceptors: PaperbackLoggerClientInterceptorFactoryProtocol? {
    return nil
  }

  public func makeStreamLogsCall(
    _ request: LogFilter,
    callOptions: CallOptions? = nil
  ) -> GRPCAsyncServerStreamingCall<LogFilter, LogLine> {
    return self.makeAsyncServerStreamingCall(
      path: PaperbackLoggerClientMetadata.Methods.streamLogs.path,
      request: request,
      callOptions: callOptions ?? self.defaultCallOptions,
      interceptors: self.interceptors?.makestreamLogsInterceptors() ?? []
    )
  }
}

@available(macOS 10.15, iOS 13, tvOS 13, watchOS 6, *)
extension PaperbackLoggerAsyncClientProtocol {
  public func streamLogs(
    _ request: LogFilter,
    callOptions: CallOptions? = nil
  ) -> GRPCAsyncResponseStream<LogLine> {
    return self.performAsyncServerStreamingCall(
      path: PaperbackLoggerClientMetadata.Methods.streamLogs.path,
      request: request,
      callOptions: callOptions ?? self.defaultCallOptions,
      interceptors: self.interceptors?.makestreamLogsInterceptors() ?? []
    )
  }
}

@available(macOS 10.15, iOS 13, tvOS 13, watchOS 6, *)
public struct PaperbackLoggerAsyncClient: PaperbackLoggerAsyncClientProtocol {
  public var channel: GRPCChannel
  public var defaultCallOptions: CallOptions
  public var interceptors: PaperbackLoggerClientInterceptorFactoryProtocol?

  public init(
    channel: GRPCChannel,
    defaultCallOptions: CallOptions = CallOptions(),
    interceptors: PaperbackLoggerClientInterceptorFactoryProtocol? = nil
  ) {
    self.channel = channel
    self.defaultCallOptions = defaultCallOptions
    self.interceptors = interceptors
  }
}

public protocol PaperbackLoggerClientInterceptorFactoryProtocol: Sendable {

  /// - Returns: Interceptors to use when invoking 'streamLogs'.
  func makestreamLogsInterceptors() -> [ClientInterceptor<LogFilter, LogLine>]
}

public enum PaperbackLoggerClientMetadata {
  public static let serviceDescriptor = GRPCServiceDescriptor(
    name: "PaperbackLogger",
    fullName: "PaperbackLogger",
    methods: [
      PaperbackLoggerClientMetadata.Methods.streamLogs,
    ]
  )

  public enum Methods {
    public static let streamLogs = GRPCMethodDescriptor(
      name: "streamLogs",
      path: "/PaperbackLogger/streamLogs",
      type: GRPCCallType.serverStreaming
    )
  }
}

/// To build a server, implement a class that conforms to this protocol.
public protocol PaperbackLoggerProvider: CallHandlerProvider {
  var interceptors: PaperbackLoggerServerInterceptorFactoryProtocol? { get }

  func streamLogs(request: LogFilter, context: StreamingResponseCallContext<LogLine>) -> EventLoopFuture<GRPCStatus>
}

extension PaperbackLoggerProvider {
  public var serviceName: Substring {
    return PaperbackLoggerServerMetadata.serviceDescriptor.fullName[...]
  }

  /// Determines, calls and returns the appropriate request handler, depending on the request's method.
  /// Returns nil for methods not handled by this service.
  public func handle(
    method name: Substring,
    context: CallHandlerContext
  ) -> GRPCServerHandlerProtocol? {
    switch name {
    case "streamLogs":
      return ServerStreamingServerHandler(
        context: context,
        requestDeserializer: ProtobufDeserializer<LogFilter>(),
        responseSerializer: ProtobufSerializer<LogLine>(),
        interceptors: self.interceptors?.makestreamLogsInterceptors() ?? [],
        userFunction: self.streamLogs(request:context:)
      )

    default:
      return nil
    }
  }
}

/// To implement a server, implement an object which conforms to this protocol.
@available(macOS 10.15, iOS 13, tvOS 13, watchOS 6, *)
public protocol PaperbackLoggerAsyncProvider: CallHandlerProvider, Sendable {
  static var serviceDescriptor: GRPCServiceDescriptor { get }
  var interceptors: PaperbackLoggerServerInterceptorFactoryProtocol? { get }

  func streamLogs(
    request: LogFilter,
    responseStream: GRPCAsyncResponseStreamWriter<LogLine>,
    context: GRPCAsyncServerCallContext
  ) async throws
}

@available(macOS 10.15, iOS 13, tvOS 13, watchOS 6, *)
extension PaperbackLoggerAsyncProvider {
  public static var serviceDescriptor: GRPCServiceDescriptor {
    return PaperbackLoggerServerMetadata.serviceDescriptor
  }

  public var serviceName: Substring {
    return PaperbackLoggerServerMetadata.serviceDescriptor.fullName[...]
  }

  public var interceptors: PaperbackLoggerServerInterceptorFactoryProtocol? {
    return nil
  }

  public func handle(
    method name: Substring,
    context: CallHandlerContext
  ) -> GRPCServerHandlerProtocol? {
    switch name {
    case "streamLogs":
      return GRPCAsyncServerHandler(
        context: context,
        requestDeserializer: ProtobufDeserializer<LogFilter>(),
        responseSerializer: ProtobufSerializer<LogLine>(),
        interceptors: self.interceptors?.makestreamLogsInterceptors() ?? [],
        wrapping: { try await self.streamLogs(request: $0, responseStream: $1, context: $2) }
      )

    default:
      return nil
    }
  }
}

public protocol PaperbackLoggerServerInterceptorFactoryProtocol: Sendable {

  /// - Returns: Interceptors to use when handling 'streamLogs'.
  ///   Defaults to calling `self.makeInterceptors()`.
  func makestreamLogsInterceptors() -> [ServerInterceptor<LogFilter, LogLine>]
}

public enum PaperbackLoggerServerMetadata {
  public static let serviceDescriptor = GRPCServiceDescriptor(
    name: "PaperbackLogger",
    fullName: "PaperbackLogger",
    methods: [
      PaperbackLoggerServerMetadata.Methods.streamLogs,
    ]
  )

  public enum Methods {
    public static let streamLogs = GRPCMethodDescriptor(
      name: "streamLogs",
      path: "/PaperbackLogger/streamLogs",
      type: GRPCCallType.serverStreaming
    )
  }
}
