...
import {
	...
	SourceTestRequest,
	SourceTestResponse,
} from "./devtools/generated/typescript/PDTSourceTester"
...
export interface ISourceTester {
	...
	testSource(
		...
		callback: (response: SourceTestResponse) => Promise<void>,
	): Promise<void>
}
...
export class SourceTester implements ISourceTester {
	...
	async testSource(
		...
		const customRunnerPath = path.join(
			process.cwd(),
			"src",
			request.sourceId,
			`${request.sourceId}.test.js`,
		)
	...
	async runTests(testCase: TestCase, testData: TestData, source: unknown) {
		...
		if (homepageProvider?.getHomePageSections) {
			await testCase(
				"source should return homepage sections containing items",
				async () => {
					...
					for (const key of sectionKeys) {
						expect(sections[key].items, `section[${key}] has no items`).to.not
							.be.empty
		...
		if (mangaProvider?.getMangaDetails) {
			await testCase(
				"source should return valid SourceManga definition",
				async () => {
					expect(
						testData.mangaId,
						"no mangaId provided and could not infer it from homepage",
					).to.exist

					const sourceManga = await mangaProvider.getMangaDetails(
						testData.mangaId!,
					)
					expect(
						sourceManga.id,
						"returned manga id does not match the queried manga id",
					).to.equal(testData.mangaId)
		...
		if (chapterProvider?.getChapters) {
			await testCase("source should return some chapters", async () => {
				...
				expect(
					chapters,
					`one or more chapters should be returned for manga '${testData.mangaId}'`,
				).to.not.be.empty
		...
		if (chapterProvider?.getChapterDetails) {
			await testCase(
				"source should return chapter details with some pages",
				async () => {
					expect(
						testData.mangaId,
						"no mangaId provided and could not infer it from homepage",
					...
					expect(
						testData.chapterId,
						"no chapterId provided and could not infer it from chapter list",
					).to.exist

					const chapterDetails = await chapterProvider.getChapterDetails(
						testData.mangaId!,
						testData.chapterId!,
					)
					expect(
						chapterDetails.id,
						"returned chapter id does not match the queried chapter id",
					).to.equal(testData.chapterId)
					expect(
						chapterDetails.mangaId,
						"returned manga id does not match the queried manga id",
					).to.equal(testData.mangaId)
					expect(chapterDetails.pages, "chapter must have pages").to.not.be
						.empty
				},
			)
		}
		...
		if (searchResultsProvider?.getSearchResults) {
			await testCase("source should return some search results", async () => {
				expect(
					testData.searchData,
					"no searchData provided and could not infer it from homepage",
				).to.exist
				...
				const searchResults = await searchResultsProvider?.getSearchResults(
					{
						...
						excludedTags:
							testData.searchData!.excludedTags?.map((x) => ({
								id: x,
								label: x,
							})) ?? [],
						includedTags:
							testData.searchData!.includedTags?.map((x) => ({
								id: x,
								label: x,
							})) ?? [],
						parameters: {},
				...
				expect(
					...
					`one or more search results should be returned for ${JSON.stringify(
						testData.searchData,
					)}`,
				).to.not.be.empty
			})
...
export class OnDeviceSourceTester implements ISourceTester {
	...
	async installSources(request: SourceInstallRequest): Promise<void> {
		await new Promise((resolve, reject) => {
			this.grpcClient.installSource(request, (error, response) =>
