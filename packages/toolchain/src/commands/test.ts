...
export default class Test extends CLICommand {
	...
	private async logSourceTestReponse(response: SourceTestResponse) {
		this.log(
			`${chalk.red.bold("#")} ${chalk.bold(response.testCase)}: ${chalk.green(
				response.completeTime + "ms",
			)}`,
		)
	...
	private getSourceIdsToTest(sourceId: string, bundleDir: string) {
		...
		sourcesToTest = sourceId
			? fs.readdirSync(bundleDir).filter((file) => {
					return (
						file.toLowerCase() === sourceId.toLowerCase() &&
						fs.statSync(path.join(bundleDir, file)).isDirectory()
					)
			  })
			: fs
	...
	private async installSources(sources: string[], client: ISourceTester) {
		...
		for (const source of sources) {
			...
			await client.installSource({
				sourceId: source,
				repoBaseUrl: `http://${ip.address()}:${server.port}`,
			})
		}
