...
export async function test(
  ...
  for (const sourceId of sourcesToTest) {
    ...
    const hasPBConfig = fs.existsSync(
      path.join(srcDirectory, sourceId, 'pbconfig.ts')
    )
  ...
  console.log(
    `  Finished in`,
    pc.dim(`${Number(buildEndTime - buildStartTime) / 1e6}ms`)
  )
  ...
  for (const scope in logData) {
    ...
    if ('tests' in logData[scope]) {
      ...
      for (const test of tests) {
        const status =
          test.status == 'pass' ? pc.green(test.status) : pc.red(test.status)
    ...
    if (flags.console && 'console' in logData[scope]) {
      output += '\nConsole:\n'
      output +=
        logData[scope].console
          .map((o: any) => `    [${o.method}] [${o.time}] ${o.args.join(' ')}`)
          .join('\n') + '\n'
    }
  }
  ...
  console.log(
    '  Duration:',
    pc.dim(`${Number(testEndTime - testStartTime) / 1e6}ms`)
  )
