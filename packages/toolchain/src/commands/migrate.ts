...
export default class Migrate extends CLICommand {
	...
	public async run(): Promise<void> {
		...
		this.log(
			`setting @paperback/types and @paperback/toolchain to ${myPkg.version}`,
		)
...
class FileTransformer {
	...
	getPropertyAssignmentExpressions<
		T extends Record<string, Expression | undefined>,
	>(node: ts.Node, obj: T): T {
	...
	getRenamedTypeWrapper(callExp: CallExpression): ts.Node | undefined {
		...
		this.changes.push(
			`Updating creation wrapper call ${expression.text} to use the App namespace`,
		)
						...
						ts.factory.createObjectLiteralExpression(
							[
								...
								ts.factory.createPropertyAssignment(
									...
									propAssignments.view_more ??
										ts.factory.createIdentifier("false"),
								),

								ts.factory.createPropertyAssignment(
									ts.factory.createIdentifier("type"),
									propAssignments.type ??
										ts.factory.createStringLiteral("singleRowNormal"),
								),
							],
							true,
						),
			...
			const visitNode = (node: ts.Node): ts.Node | undefined => {
				...
				if (ts.isImportDeclaration(node)) {
					...
					if (
						...
						moduleSpecifier.text.startsWith("paperback-extensions-common")
					) {
								...
								ts.factory.createImportClause(
									...
									namedImports?.elements
										? ts.factory.createNamedImports(
												namedImports.elements.map((x) =>
													this.getRenamedImport(x),
												),
										  )
				...
				if (ts.isCallExpression(node)) {
					...
					if (ts.isIdentifier(expression)) {
						return ts.visitEachChild(
							this.getRenamedTypeWrapper(node),
							visitNode.bind(this),
							context,
						)
					}
				...
				if (ts.isPropertyAccessExpression(node)) {
					return ts.visitEachChild(
						this.getRenamedPropertyAccessor(node),
						visitNode.bind(this),
						context,
					)
				}
