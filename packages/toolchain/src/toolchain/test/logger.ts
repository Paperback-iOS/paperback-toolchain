...
export class Logger implements TestLogger {
    ...
    private get _obj(): ObjectNode {
        if (this._isArray) throw new Error("This logger wraps a list — call .scope(name) to create an entry first.");
        return this._node as ObjectNode;
    }

    private get _arr(): ArrayNode {
        if (!this._isArray) throw new Error("This logger wraps an object — use .list(name) to get a list logger.");
